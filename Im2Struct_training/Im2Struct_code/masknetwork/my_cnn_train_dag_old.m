function [net,stats] = my_cnn_train_dag_old(imdb, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(imdb.nyudata.set==1) ; end
if isempty(opts.val), opts.val = find(imdb.nyudata.set==2) ; end
...
if exist(fullfile(opts.saveDir, sprintf('FT-net-%d.mat', opts.id)), 'file')
   ...
   if exist(fullfile(opts.dataDir, 'start-net-final-05.mat'), 'file')
        net = loadState(fullfile(opts.dataDir, 'start-net-final-05.mat')) ;
        net.meta.imgRbgMean = opts.imgRbgMean;
        fprintf('load start net model \n');
   else
       net = myDAGnet;
       net.meta.imgRbgMean = opts.imgRbgMean;
       saveState(fullfile(opts.dataDir, 'start-net-final-05.mat'), net, [], 0);
   end
end
...
for epoch=epoch_start+1:opts.numEpochs
  ...
  net.layers(58).block.setClip(opts.clip_d(epoch));
...
net.layers(59).block.recalerror();
stats = opts.extractStatsFn(net) ;
