...
net.addLayer('scale2Conv1', ...
     dagnn.Conv('size', [9 9 3 96], 'pad', [0 0 0 0], 'stride', [2 2]), ...
     'input2', 'x43', {'scale2Conv1_f','scale2Conv1_b'});

f = net.getParamIndex('scale2Conv1_f') ;
net.params(f).value = 0.01*randn(9, 9, 3, 96, 'single') ;
net.params(f).learningRate = 1 ;
net.params(f).weightDecay = 1 ;
...
net.addLayer('pool1_scale2', dagnn.Pooling('poolSize', [3 3], 'stride', [2 2]), 'x44', 'xfix', {}) ;
...
net.addLayer('scale2Conv6', ...
     dagnn.Conv('size', [5 5 65 64], 'pad', [2 2 2 2]), ...
     'x55', 'x56', {'scale2Conv6_f','scale2Conv6_b'});

f = net.getParamIndex('scale2Conv6_f') ;
net.params(f).value = 0.01*randn(5, 5, 65, 64, 'single') ;
net.params(f).learningRate = 1 ;
net.params(f).weightDecay = 1 ;
...
net.addLayer('scale2Conv10', ...
     dagnn.Conv('size', [5 5 64 1], 'pad', [2 2 2 2]), ...
     'x63', 'prediction', {'scale2Conv10_f','scale2Conv10_b'});

f = net.getParamIndex('scale2Conv10_f') ;
net.params(f).value = 0.01*randn(5, 5, 64, 1, 'single') ;
net.params(f).learningRate = 1 ;
net.params(f).weightDecay = 1 ;
