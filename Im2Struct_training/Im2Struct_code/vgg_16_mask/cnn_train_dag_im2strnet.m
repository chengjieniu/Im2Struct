function [net,stats] = cnn_train_dag_im2strnet(net, imdb, getBatch, varargin)
...
%modelVaePath = @(ep) fullfile(opts.expDir, sprintf('net-vae_theta-%d.mat', ep));
...
%   
  ...
  for ii=1:36
        net.params(ii).value=gpuArray( net.params(ii).value);
  end
  
  if numel(opts.gpus) <= 1
    [~, net, state] = processEpoch(net.meta.vae_theta, net, state, params, epoch, 'train') ;
    [vae_theta, net, state] = processEpoch(net.meta.vae_theta, net, state, params, epoch, 'val') ;
    if ~evaluateMode
        if mod(epoch,5)==0
             saveState(modelPath(epoch), net, state) ;
             %vae_theta已经存入到net
%             save(modelVaePath(epoch),'net.meta.vae_theta');
        end
    end
...
%save D:\exp_im2str\vae_theta\vae_theta vae_theta;
% With multiple GPUs, return one copy
if isa(net, 'Composite'), net = net{1} ; end

% -------------------------------------------------------------------------
function [vae_theta, net, state] = processEpoch(vae_theta, net, state, params, epoch, mode)
% -------------------------------------------------------------------------
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    if strcmp(mode, 'train')
      ...
      net.eval(inputs, params.derOutputs,  net.meta.vae_theta, epoch,'holdOn', s < params.numSubBatches) ;
    else
      net.mode = 'test' ;
      net.eval(inputs, params.derOutputs,  net.meta.vae_theta, epoch) ;
    end
  end
...
%       thisLR = params.learningRate * net.params(p).learningRate ;
      ...
      thisDecay = net.params(p).weightDecay ;
      thisLR =  net.params(p).learningRate ;
...
for i = 1:numel(sel)
  ...
  stats.('reconLoss')=net.layers(sel(i)).block.average(1)/net.meta.trainOpts.batchSize;
  stats.('symLoss')=net.layers(sel(i)).block.average(2)/net.meta.trainOpts.batchSize;
  stats.('catLoss')=net.layers(sel(i)).block.average(3)/net.meta.trainOpts.batchSize;
%   else
